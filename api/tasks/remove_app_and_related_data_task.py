...
def remove_app_and_related_data_task(self, tenant_id: str, app_id: str):
    logging.info(
        click.style(
            f"Start deleting app and related data: {tenant_id}:{app_id}", fg="green"
        )
    )
    ...
    try:
        ...
        logging.info(
            click.style(
                f"App and related data deleted: {app_id} latency: {end_at - start_at}",
                fg="green",
            )
        )
        ...
        logging.exception(
            click.style(
                f"Database error occurred while deleting app {app_id} and related data",
                fg="red",
            )
        )
        ...
        logging.exception(
            click.style(
                f"Error occurred while deleting app {app_id} and related data", fg="red"
            )
        )
        raise self.retry(exc=e, countdown=60)  # Retry after 60 seconds
...
def _delete_app_site(tenant_id: str, app_id: str):
    def del_site(site_id: str):
        db.session.query(Site).filter(Site.id == site_id).delete(
...
def _delete_app_api_tokens(tenant_id: str, app_id: str):
    def del_api_token(api_token_id: str):
        db.session.query(ApiToken).filter(ApiToken.id == api_token_id).delete(
            synchronize_session=False
        )
...
def _delete_installed_apps(tenant_id: str, app_id: str):
    def del_installed_app(installed_app_id: str):
        db.session.query(InstalledApp).filter(
...
def _delete_recommended_apps(tenant_id: str, app_id: str):
    def del_recommended_app(recommended_app_id: str):
        db.session.query(RecommendedApp).filter(
            RecommendedApp.id == recommended_app_id
        ).delete(synchronize_session=False)
...
def _delete_app_dataset_joins(tenant_id: str, app_id: str):
    def del_dataset_join(dataset_join_id: str):
        db.session.query(AppDatasetJoin).filter(
            AppDatasetJoin.id == dataset_join_id
        ).delete(synchronize_session=False)
...
def _delete_app_workflows(tenant_id: str, app_id: str):
    def del_workflow(workflow_id: str):
        db.session.query(Workflow).filter(Workflow.id == workflow_id).delete(
            synchronize_session=False
        )
...
def _delete_app_workflow_runs(tenant_id: str, app_id: str):
    def del_workflow_run(workflow_run_id: str):
        db.session.query(WorkflowRun).filter(WorkflowRun.id == workflow_run_id).delete(
            synchronize_session=False
        )
...
def _delete_app_workflow_node_executions(tenant_id: str, app_id: str):
    def del_workflow_node_execution(workflow_node_execution_id: str):
        db.session.query(WorkflowNodeExecution).filter(
            WorkflowNodeExecution.id == workflow_node_execution_id
        ).delete(synchronize_session=False)
...
def _delete_app_workflow_app_logs(tenant_id: str, app_id: str):
    def del_workflow_app_log(workflow_app_log_id: str):
        db.session.query(WorkflowAppLog).filter(
            WorkflowAppLog.id == workflow_app_log_id
        ).delete(synchronize_session=False)
...
def _delete_workflow_tool_providers(tenant_id: str, app_id: str):
    def del_tool_provider(tool_provider_id: str):
        db.session.query(WorkflowToolProvider).filter(
            WorkflowToolProvider.id == tool_provider_id
        ).delete(synchronize_session=False)
...
def _delete_app_tag_bindings(tenant_id: str, app_id: str):
    def del_tag_binding(tag_binding_id: str):
        db.session.query(TagBinding).filter(TagBinding.id == tag_binding_id).delete(
            synchronize_session=False
        )
...
def _delete_end_users(tenant_id: str, app_id: str):
    def del_end_user(end_user_id: str):
        db.session.query(EndUser).filter(EndUser.id == end_user_id).delete(
            synchronize_session=False
        )
...
def _delete_trace_app_configs(tenant_id: str, app_id: str):
    def del_trace_app_config(trace_app_config_id: str):
        db.session.query(TraceAppConfig).filter(
            TraceAppConfig.id == trace_app_config_id
        ).delete(synchronize_session=False)
...
def _delete_records(
    query_sql: str, params: dict, delete_func: Callable, name: str
) -> None:
    while True:
        with db.engine.begin() as conn:
            ...
            for i in rs:
                ...
                try:
                    ...
                    logging.exception(
                        f"Error occurred while deleting {name} {record_id}"
                    )
                    continue
            rs.close()
