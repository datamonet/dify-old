...
def external_document_indexing_task(
    ...
    if not dataset:
        logging.info(
            click.style(
                "Processed external dataset: {} failed, dataset not exit.".format(
                    dataset_id
                ),
                fg="red",
            )
    ...
    external_knowledge_api = (
        ...
        .filter(
            ExternalKnowledgeApis.id == external_knowledge_api_id,
    ...
    if data_source["type"] == "upload_file":
        ...
        for file_id in upload_file_list:
            file = (
                ...
                .filter(
                    UploadFile.tenant_id == dataset.tenant_id, UploadFile.id == file_id
                )
                .first()
            )
            if file:
    ...
    try:
        ...
        # assemble headers
        headers = ExternalDatasetService.assembling_headers(
            settings.authorization, settings.headers
        )

        # do http request
        response = ExternalDatasetService.process_external_api(
            settings, headers, process_parameter, files
        )
        ...
        logging.info(
            click.style(
                "Processed external dataset: {} successful, latency: {}".format(
                    dataset.id, end_at - start_at
                ),
                fg="green",
            )
        )
