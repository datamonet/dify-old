...
from ._legacy_binary_response import (
    HttpxResponseContent,
    HttpxTextBinaryResponseContent,
)
...
class LegacyAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if self._stream:
            if to:
                if not is_stream_class_type(to):
                    raise TypeError(
                        f"Expected custom parse type to be a subclass of {StreamResponse}"
                    )
            ...
            stream_cls = cast(
                "type[StreamResponse[Any]] | None", self._client._default_stream_cls
            )
        ...
        if inspect.isclass(origin) and issubclass(origin, HttpxResponseContent):
            ...
            *_, filename = response.headers.get("content-disposition", "").split(
                "filename="
            )
        ...
        if inspect.isclass(origin) and issubclass(origin, httpx.Response):
            ...
            if cast_type != httpx.Response:
                raise ValueError(
                    "Subclasses of httpx.Response cannot be passed to `cast_type`"
                )
