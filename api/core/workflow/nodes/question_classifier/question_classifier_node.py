...
class QuestionClassifierNode(LLMNode):
    ...
    def _run(self) -> NodeRunResult:
        node_data: QuestionClassifierNodeData = cast(
            self._node_data_cls, self.node_data
        )
        ...
        # fetch instruction
        instruction = (
            self._format_instruction(node_data.instruction, variable_pool)
    ...
    def _format_instruction(self, instruction: str, variable_pool: VariablePool) -> str:
        ...
        prompt_template = PromptTemplateParser(
            template=instruction, with_variable_tmpl=True
        )
        prompt_inputs = {
