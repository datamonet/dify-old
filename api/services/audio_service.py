...
class AudioService:
    @classmethod
    def transcript_asr(
        cls, app_model: App, file: FileStorage, end_user: Optional[str] = None
    ):
        if app_model.mode in {AppMode.ADVANCED_CHAT.value, AppMode.WORKFLOW.value}:
            ...
            if "speech_to_text" not in features_dict or not features_dict[
                "speech_to_text"
            ].get("enabled"):
                raise ValueError("Speech to text is not enabled")
        else:
        ...
        def invoke_tts(text_content: str, app_model, voice: Optional[str] = None):
            with app.app_context():
                if app_model.mode in {
                    ...
                    features_dict = workflow.features_dict
                    if "text_to_speech" not in features_dict or not features_dict[
                        "text_to_speech"
                    ].get("enabled"):
                        raise ValueError("TTS is not enabled")
        ...
        if message_id:
            ...
            if message.answer == "" and message.status == "normal":
                ...
                if isinstance(response, Generator):
                    return Response(
                        stream_with_context(response), content_type="audio/mpeg"
                    )
                return response
        else:
    ...
    def transcript_tts_voices(cls, tenant_id: str, language: str):
        model_manager = ModelManager()
        model_instance = model_manager.get_default_model_instance(
            tenant_id=tenant_id, model_type=ModelType.TTS
        )
        if model_instance is None:
