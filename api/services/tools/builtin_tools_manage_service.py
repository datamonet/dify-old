...
class BuiltinToolManageService:
    ...
    def list_builtin_tool_provider_tools(
        ...
        """
        provider_controller: ToolProviderController = ToolManager.get_builtin_provider(
            provider
        )
        ...
        if builtin_provider is not None:
            ...
            credentials = tool_provider_configurations.decrypt_tool_credentials(
                credentials
            )
    ...
    def list_builtin_tools(user_id: str, tenant_id: str) -> list[UserToolProvider]:
        ...
        find_provider = lambda provider: next(
            filter(lambda db_provider: db_provider.provider == provider, db_providers),
            None,
        )
                ...
                for tool in tools:
                    user_builtin_provider.tools.append(
                        ToolTransformService.tool_to_user_tool(
                            ...
                            labels=ToolLabelManager.get_tool_labels(
                                provider_controller
                            ),
                        )
                    )
