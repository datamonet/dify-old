...
def test_segment_group_to_text():
    ...
    segments_group = parser.convert_template(
        template=template, variable_pool=variable_pool
    )
    segments_group = variable_pool.convert_template(template)

    assert (
        segments_group.text
        == "Hello, fake-user-id! Your query is fake-user-query. And your key is fake-secret-key."
    )
