...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "disabled", sa.Boolean(), server_default=sa.text("false"), nullable=True
        ),
        sa.PrimaryKeyConstraint("id", name="data_source_api_key_auth_binding_pkey"),
    )
    with op.batch_alter_table(
        "data_source_api_key_auth_bindings", schema=None
    ) as batch_op:
        batch_op.create_index(
            "data_source_api_key_auth_binding_provider_idx", ["provider"], unique=False
        )
        batch_op.create_index(
    ...
    with op.batch_alter_table("data_source_oauth_bindings", schema=None) as batch_op:
        batch_op.create_index(
            "source_binding_tenant_id_idx", ["tenant_id"], unique=False
        )
        batch_op.create_index(
            "source_info_idx", ["source_info"], unique=False, postgresql_using="gin"
        )
