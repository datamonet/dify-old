...
def upgrade():
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint(
            "id", "conversation_id", name=op.f("workflow__conversation_variables_pkey")
        ),
    )
    with op.batch_alter_table(
        "workflow__conversation_variables", schema=None
    ) as batch_op:
        batch_op.create_index(
            batch_op.f("workflow__conversation_variables_app_id_idx"),
            ["app_id"],
            unique=False,
        )
        batch_op.create_index(
            batch_op.f("workflow__conversation_variables_created_at_idx"),
            ["created_at"],
            unique=False,
        )
    ...
    with op.batch_alter_table("workflows", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "conversation_variables", sa.Text(), server_default="{}", nullable=False
            )
        )
