...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "score_threshold", sa.Float(), server_default=sa.text("0"), nullable=False
        ),
    ...
    with op.batch_alter_table("app_annotation_settings", schema=None) as batch_op:
        batch_op.create_index(
            "app_annotation_settings_app_idx", ["app_id"], unique=False
        )
...
def downgrade():
    ...
    with op.batch_alter_table("app_model_configs", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("annotation_reply", sa.TEXT(), autoincrement=False, nullable=True)
        )
