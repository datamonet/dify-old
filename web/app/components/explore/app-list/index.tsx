...
const Apps = ({ pageType = PageType.EXPLORE, onSuccess }: AppsProps) => {
  ...
  const {
    ...
    () =>
      fetchAppList().then(({ categories, community, recommended_apps }) => ({
        ...
        community,
        recommended_apps,
        allList: [...community, ...recommended_apps].sort(
  ...
  const filteredList = useMemo(() => {
    if (currCategory === allCategoriesEn) {
      if (!currentType) return recommended_apps;
      ...
      if (!currentType && currCategory === "community") return community;
  ...
  const searchFilteredList = useMemo(() => {
    ...
    const idMatchedItems = filteredList.filter(
      (item) => searchKeywords === item.app.id
    );
  ...
  useEffect(() => {
    ...
    if (anchorRef.current && currCategory === "favourite") {
      observer = new IntersectionObserver(
        (entries) => {
          if (entries[0].isIntersecting) setSize((size: number) => size + 1);
        },
        { rootMargin: "100px" }
      );
