...
import { emailAWS } from "@/service/email";
...
const StepTwo = ({
  ...
  const [segmentIdentifier, doSetSegmentIdentifier] = useState(
  ...
  const [automaticFileIndexingEstimate, setAutomaticFileIndexingEstimate] =
    useState<FileIndexingEstimateResponse | null>(null);
  // takin command: 计算QA
  const [estimateTokes, setEstimateTokes] = useState<Pick<
    IndexingEstimateResponse,
    "tokens" | "total_price"
  > | null>(null);
  ...
  const scrollHandle = (e: Event) => {
    if ((e.target as HTMLDivElement).scrollTop > 0) setScrolled(true);
    else setScrolled(false);
  };

  const previewScrollHandle = (e: Event) => {
    if ((e.target as HTMLDivElement).scrollTop > 0) setPreviewScrolled(true);
    else setPreviewScrolled(false);
  };
  ...
  const confirmChangeCustomConfig = () => {
    if (segmentationType === SegmentType.CUSTOM && max > 4000) {
      Toast.notify({
        type: "error",
        message: t("datasetCreation.stepTwo.maxLengthCheck"),
      });
      return;
    }
  ...
  const createHandle = async () => {
    ...
    try {
      ...
      if (mutateDatasetRes) mutateDatasetRes();
      ...
      try {
        ...
        if (response) cost = response.total_price;
      } catch (err) {
        console.log(err);
        cost = 1;
        await emailAWS({
  ...
  const handleSwitch = (state: boolean) => {
    if (state) setDocForm(DocForm.QA);
    else setDocForm(DocForm.TEXT);
  };
  ...
  const previewSwitch = async () => {
    ...
    else setCustomFileIndexingEstimate(null);
  ...
  useLayoutEffect(() => {
    if (showPreview) {
      ...
      return () => {
        previewScrollRef.current?.removeEventListener(
          "scroll",
          previewScrollHandle
        );
      };
  ...
  useEffect(() => {
    ...
    if (indexingType) setIndexType(indexingType as IndexingType);
    else
      setIndexType(
  ...
  return (
    <div className="flex w-full h-full">
      <div ref={scrollRef} className="relative h-full w-full overflow-y-scroll">
        <div
          className={cn(s.pageHeader, scrolled && s.fixed, isMobile && "!px-6")}
        >
                    ...
                    <div className="w-full flex flex-col gap-1">
                      ...
                      {rules.map((rule) => (
                        <div key={rule.id} className={s.ruleItem}>
                          ...
                          <label
                            htmlFor={rule.id}
                            className="ml-2 text-sm font-normal cursor-pointer text-gray-800"
                          >
                            {getRuleName(rule.id)}
                          </label>
                        </div>
          ...
          <div className="max-w-[640px]">
            <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
              {(!hasSetIndexType ||
                ...
                <div
                  ...
                  onClick={() => {
                    if (isAPIKeySet) setIndexType(IndexingType.QUALIFIED);
                  }}
                >
                  ...
                  {!isAPIKeySet && (
                    <div className={s.warningTip}>
                      ...
                      <span className={s.click} onClick={onSetting}>
              ...
              {(!hasSetIndexType ||
                ...
                <div
                  className={cn(
                    ...
                    s.indexItem,
                    !hasSetIndexType &&
                      indexType === IndexingType.ECONOMICAL &&
                      s.active,
                    hasSetIndexType && s.disabled,
                    hasSetIndexType && "!w-full !min-h-[96px]"
                  )}
              ...
              <div className="mt-3 rounded-xl bg-gray-50 border border-gray-100">
                <div className="flex justify-between items-center px-5 py-4">
                  ...
                  <div className="grow mx-3">
                    <div className="mb-[2px] text-md font-medium text-gray-900">
            ...
            {indexType === IndexingType.QUALIFIED && (
              <div className="mb-2">
                <div
                  className={cn(
                    s.label,
                    datasetId && "flex justify-between items-center"
                  )}
                >
            ...
            <div>
              {!datasetId ? (
                ...
                <div
                  className={cn(s.label, "flex justify-between items-center")}
                >
            ...
            {!isSetting ? (
              <div className="flex items-center mt-8 py-2">
                ...
                >
                  {t("datasetCreation.stepTwo.nextStep")}
                </Button>
              </div>
            ) : (
              <div className="flex items-center mt-8 py-2">
                <Button
                  ...
                  onClick={createHandle}
                >
                  {t("datasetCreation.stepTwo.save")}
                </Button>
              ...
              <div className="flex items-center justify-between px-8">
                <div className="grow flex items-center">
                  ...
                  {docForm === DocForm.QA && !previewSwitched && (
                    ...
                    >
                      {t("datasetCreation.stepTwo.previewButton")}
                    </Button>
                  )}
                </div>
                <div
