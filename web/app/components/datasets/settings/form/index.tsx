...
const Form = () => {
  ...
  const { dataset: currentDataset, mutateDatasetRes: mutateDatasets } =
  ...
  const [topK, setTopK] = useState(
    currentDataset?.external_retrieval_model.top_k ?? 2
  );
  ...
  const getMembers = async () => {
    ...
    if (!accounts) setMemberList([]);
    else setMemberList(accounts);
  };
  ...
  const handleSave = async () => {
    ...
    if (postRetrievalConfig.weights) {
      postRetrievalConfig.weights.vector_setting.embedding_provider_name =
        currentDataset?.embedding_model_provider || "";
      postRetrievalConfig.weights.vector_setting.embedding_model_name =
        currentDataset?.embedding_model || "";
    }
    try {
      setLoading(true);
      const requestParams = {
        ...
        body: {
          ...
          retrieval_model: {
            ...postRetrievalConfig,
            score_threshold: postRetrievalConfig.score_threshold_enabled
              ? postRetrievalConfig.score_threshold
              : 0,
          },
  ...
  return (
    <div className="w-full sm:w-[800px] p-4 sm:px-16 sm:py-6">
      ...
      <div className={rowClass}>
        <div className={labelClass} />
        <div className="w-[480px]">
          <Button className="min-w-24" variant="primary" onClick={handleSave}>
