...
export type ModalContextState = {
  setShowAccountSettingModal: Dispatch<
    SetStateAction<ModalState<string> | null>
  >;
  setShowApiBasedExtensionModal: Dispatch<
    SetStateAction<ModalState<ApiBasedExtension> | null>
  >;
  setShowModerationSettingModal: Dispatch<
    SetStateAction<ModalState<ModerationConfig> | null>
  >;
  setShowExternalDataToolModal: Dispatch<
    SetStateAction<ModalState<ExternalDataTool> | null>
  >;
  setShowPricingModal: () => void;
  setShowAnnotationFullModal: () => void;
  setShowCreditsBillingModal: () => void;
  setShowModelModal: Dispatch<
    SetStateAction<ModalState<ModelModalType> | null>
  >;
  ...
  setShowModelLoadBalancingModal: Dispatch<
    SetStateAction<ModelLoadBalancingModalProps | null>
  >;
  setShowModelLoadBalancingEntryModal: Dispatch<
    SetStateAction<ModalState<LoadBalancingEntryModalType> | null>
  >;
...
export const ModalContextProvider = ({
  ...
  const [showAccountSettingModal, setShowAccountSettingModal] =
    useState<ModalState<string> | null>(null);
  ...
  const [showModelModal, setShowModelModal] =
    useState<ModalState<ModelModalType> | null>(null);
  ...
  const [showCreditsBillingModal, setShowCreditsBillingModal] =
  ...
  const [showPricingModal, setShowPricingModal] = useState(
    searchParams.get("show-pricing") === "1"
  );
  ...
  const handleRemoveModelLoadBalancingEntry = useCallback(() => {
    showModelLoadBalancingEntryModal?.onRemoveCallback?.(
      showModelLoadBalancingEntryModal.payload
    );
    setShowModelLoadBalancingEntryModal(null);
  }, [showModelLoadBalancingEntryModal]);
  ...
  const handleSaveApiBasedExtension = (
    newApiBasedExtension: ApiBasedExtension
  ) => {
  ...
  const handleSaveExternalDataTool = (
    newExternalDataTool: ExternalDataTool
  ) => {
  ...
  const handleValidateBeforeSaveExternalDataTool = (
    newExternalDataTool: ExternalDataTool
  ) => {
    if (showExternalDataToolModal?.onValidateBeforeSaveCallback)
      return showExternalDataToolModal?.onValidateBeforeSaveCallback(
        newExternalDataTool
      );
    return true;
  };
      ...
      <>
        ...
        {!!showModelModal && (
          <ModelModal
            ...
            configurateMethod={
              showModelModal.payload.currentConfigurationMethod
            }
        ...
        {!!showExternalKnowledgeAPIModal && (
          <ExternalAPIModal
            ...
            datasetBindings={
              showExternalKnowledgeAPIModal.datasetBindings ?? []
            }
        ...
        {showOpeningModal && (
          <OpeningSettingModal
            ...
            onAutoAddPromptVariable={
              showOpeningModal.payload.onAutoAddPromptVariable
            }
          />
        )}
      </>
